...
```{r}
name <- c("strawberry", "apple", "bannana", "grapes")
price <- c(0.97, 0.69, 0.83, 0.89)
quantity <- c(3,2,4,2)
...
gap <- read.csv(file = "gapminder-FiveYearData.csv", 
...
```{r}
sleep_VIM <- read.csv(file = "sleep_VIM.csv")
sleep_VIM
```
...
```{r}
heart_subset <- heart[ , c("sex", "trestbps", "target")]
head(heart_subset)
```
...
```{r}
heart_subset_int <- heart[c(3:7, 12), c(2, 4, 14)]
heart_subset_int
```
...
df1 <- data.frame(Name = c("Joe", "Susan", "Jack", "Kelly"),
                  City = c("Berkeley", "Berkeley", 
                           "Oakland", "Oakland"), 
                  Math = c(42, 48, 50,46),
                  Reading = c(10, 10, 8, 10)) 
df1

## And a second toy dataframe:

df2 <- data.frame(Science = c(99, 100, 99, 100), 
                  Music = c(19, 18, 20, 20), 
                  Art = c(20, 20, 19, 18),
                  Name = c("Joe", "Susan", "Jack", "Kelly"))
df2
...
```{r}
merge_df <- merge(df_merge, challenge_df, by = "Name")
merge_df
```
...
```{r}
inner_df <- inner_join(df_merge, challenge_df, by = "Name")
inner_df

two_inner_df <- inner_join(df_merge, challenge_df, by = c("Name", "City"))
two_inner_df
```
...
```{r}
left_df <- left_join(df_merge, challenge_df, by = c("Name", "City"))
left_df

right_df <- right_join(df_merge, challenge_df, by = c("Name", "City"))
right_df
```
...
```{r}
full_df <- full_join(df_merge, challenge_df, by = c("Name", "City"))
full_df
```
